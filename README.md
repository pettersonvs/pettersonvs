# 👋 Olá, eu sou o Petterson Neves

🚀 Desenvolvedor Python | Backend | Automações | Flask  
💻 Experiência em TI, suporte técnico e desenvolvimento de aplicações web  
📊 Interesse em dados, automação de processos e soluções práticas  
🧠 Formação em Psicologia, unindo tecnologia e comportamento humano

---

## 🧩 Sobre mim

Sou um profissional com background em **TI, desenvolvimento em Python e suporte técnico**, atualmente focado em criar **aplicações web, automações e soluções que resolvem problemas reais**.

Tenho experiência com:
- Desenvolvimento backend em **Python**
- Criação de aplicações web com **Flask**
- Manipulação e conversão de arquivos (PDF, Excel, DOCX)
- Suporte técnico e análise de sistemas
- Organização de projetos e boas práticas de código

---

## 🛠️ Tecnologias e Ferramentas

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![Flask](https://img.shields.io/badge/Flask-000000?style=for-the-badge&logo=flask&logoColor=white)
![HTML](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)

![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black)
![Windows](https://img.shields.io/badge/Windows-0078D6?style=for-the-badge&logo=windows&logoColor=white)

---

## 📂 Projetos em destaque

🔹 **Conversor PDF → Excel**  
Aplicação Flask que extrai dados de PDFs e gera planilhas automatizadas.

🔹 **Sistema Web de Consulta de Salas**  
Aplicação para alunos consultarem a sala da aula do dia, com backend em Python.

🔹 **IA para devocionais**  
Projeto em python para auxiliar iniciantes na jornada da fé


---

## 📫 Contato

📧 Email: petterson1998@yahoo.com  
💼 LinkedIn: [https://linkedin.com/in/petterson-neves](https://www.linkedin.com/in/petterson-neves/)

---

⭐ Se curtir algum projeto, não esqueça de deixar uma estrela!
